...
public class DtoBodyDeletion_ActionMap {
  ...
  /*package*/ static AbstractCellAction createAction_DELETE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        if (!(ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.components$c20O)).all(new IWhereFilter<SNode>() {
          public boolean accept(SNode it) {
            return SNodeOperations.isInstanceOf(it, CONCEPTS.EmptyLine$_9);
          }
        })) && DeletionApproverUtil.approve(editorContext, node, "objectBody")) {
          return;
        }

        ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.components$c20O)).clear();
        SelectionUtil.selectLabelCellWithSelection(editorContext, node, SelectionManager.LAST_CELL, -1, -1);
      }
      @Override
      ...
      public boolean canExecute_internal(EditorContext editorContext, SNode node) {
        return ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.components$c20O)).isNotEmpty();
      }
