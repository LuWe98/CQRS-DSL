...
public final class EntitiyKotlinOutput__BehaviorDescriptor extends BaseBHDescriptor {
  private static final SAbstractConcept CONCEPT = MetaAdapterFactory.getConcept(0x21c0c49f06c045cbL, 0x990e6280d48112f6L, 0x42e485aea7525531L, "CQRS.Kotlin.structure.EntitiyKotlinOutput");
  ...
  /*package*/ static Set<DependencyImport> getImports_id4yUYSn0sW9K(@NotNull SNode __thisNode__) {
    ...
    SetSequence.fromSet(imports).addElement(resolver.resolveEntityBaseClass());
    SetSequence.fromSet(imports).addSequence(SetSequence.fromSet(resolver.resolveType(SLinkOperations.getTarget(SLinkOperations.getTarget(__thisNode__, LINKS.entity$OPtA), LINKS.idType$P983))));
    SetSequence.fromSet(imports).addSequence(ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.parameters$j0A8)).translate(new ITranslator2<SNode, DependencyImport>() {
    ...
    SetSequence.fromSet(imports).addSequence(Sequence.fromIterable(Entity__BehaviorDescriptor.properties_id3DbZR$NYkue.invoke(SLinkOperations.getTarget(__thisNode__, LINKS.entity$OPtA))).translate(new ITranslator2<SNode, DependencyImport>() {
      public Iterable<DependencyImport> translate(SNode it) {
        return resolver.resolveType(SLinkOperations.getTarget(it, LINKS.type$AxAW));
      }
    }));
    SetSequence.fromSet(imports).addSequence(ListSequence.fromList(SLinkOperations.getChildren(__thisNode__, LINKS.methods$j5Ku)).translate(new ITranslator2<SNode, DependencyImport>() {
      public Iterable<DependencyImport> translate(SNode it) {
        return (Set<DependencyImport>) ComponentOutput__BehaviorDescriptor.getImports_id4yUYSn0sW9K.invoke(it);
      }
    }));
  ...
  /*package*/ EntitiyKotlinOutput__BehaviorDescriptor() {
  }
